...
class CrossPoster {
    ...
    try {
      ...
      try {
        ...
        return {
          ...
          content: postContent,
          tags: Array.isArray(frontmatter.tags) ? frontmatter.tags as string[] : (frontmatter.tags as string).split(',').map((t: string) => t.trim()).filter(Boolean),
   ...
   */
  async publishToAll(slug: string, canonicalUrl: string, publishStatus: 'draft' | 'unlisted' | 'public' = 'draft'): Promise<CrossPostResult[]> {
...
if (require.main === module) {
  ...
  if (args.length < 2) {
    ...
    console.error('Example: tsx scripts/cross-post.ts my-awesome-post https://yoursite.com/posts/my-awesome-post draft');
  ...
  crossPoster.publishToAll(postSlug, canonicalUrl, publishStatus as 'draft' | 'unlisted' | 'public')
    .then(results => {
