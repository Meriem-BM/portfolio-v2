...
async function testSetup() {
  ...
  if (config.platforms.hashnode?.enabled) {
    ...
    try {
      ...
      if (response.data?.publication) {
        console.log(
          `✅ Hashnode: Connected to "${(response.data as { publication: { title: string } }).publication.title}"`
        );
      } else {
  ...
  console.log(
    `Enabled platforms: ${enabledPlatforms.length > 0 ? enabledPlatforms.join(", ") : "None"}`
  );
