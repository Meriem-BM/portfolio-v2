...
import { ArrowLeft, Calendar, Tag, Heart, MessageCircle, Share2, Bookmark, Code, Zap, Terminal } from "lucide-react"
import { Button } from "@/components/ui/button"
import { Card, CardContent } from "@/components/ui/card"

// Enhanced blog post data with rich content components
const blogPosts = {
  "building-with-viem": {
    id: 1,
...
async function getTokenBalance(address: string, tokenAddress: string) {
  ...
  return formatUnits(balance, 18)
      ...
      },
      {
        type: "callout",
        variant: "success",
        content: "The migration wasn't without challenges, but the improved developer experience made it worthwhile.",
      },
    ],
  },
  "experimental-ui-art": {
    ...
    content: [
      ...
      {
        type: "interactive",
        content: "Hover over this text to see experimental interactions in action!",
      },
      {
        type: "callout",
        variant: "warning",
        content: "Remember: Innovation should enhance, not hinder, the user experience.",
      },
    ],
  },
      ...
      },
      {
        ...
        items: [
          ...
          },
          {
            ...
            description: "Smart contracts, frontend scaffolding, API integration",
          },
          {
            time: "Hours 24-42",
...
const ContentRenderer = ({ item, index }: { item: any; index: number }) => {
  ...
  const baseClasses = `transition-all duration-500 ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"}`
  ...
  switch (item.type) {
      ...
      return (
        <div className={`${baseClasses} mb-8`}>
          <div className="relative p-8 rounded-lg bg-gradient-to-r from-purple-900/20 via-blue-900/20 to-green-900/20 border border-white/10">
            ...
            <p className="relative text-xl leading-relaxed text-white/90 font-light">{item.content}</p>
          </div>
        </div>
      )
    ...
    case "heading":
      return (
        <h2 className={`${baseClasses} text-3xl font-bold mb-6 flex items-center gap-3`}>
    ...
    case "text":
      return <p className={`${baseClasses} text-white/80 leading-relaxed mb-6 text-lg`}>{item.content}</p>
          ...
          <Card className="bg-black/80 border border-green-500/30 overflow-hidden">
            ...
            <CardContent className="p-0">
              <pre className="p-6 overflow-x-auto">
                <code className="text-green-300 font-mono text-sm leading-relaxed">{item.content}</code>
              </pre>
            </CardContent>
          </Card>
        </div>
    ...
    case "callout":
      const variants = {
        ...
        warning: { bg: "bg-yellow-500/10", border: "border-yellow-500/30", text: "text-yellow-300", icon: "⚠️" },
      }
      const variant = variants[item.variant as keyof typeof variants] || variants.info
        ...
        <div className={`${baseClasses} mb-8`}>
          <div className="relative">
            ...
            <div className="space-y-6">
              {item.items.map((timelineItem: any, i: number) => (
                <div key={i} className="relative flex items-start gap-6">
                  ...
                  <Card className="flex-1 bg-black/50 border border-white/10 hover:border-green-500/30 transition-colors">
                    <CardContent className="p-4">
                      <div className="font-mono text-sm text-green-400 mb-1">{timelineItem.time}</div>
                      <h4 className="font-bold mb-2">{timelineItem.title}</h4>
                      <p className="text-white/70 text-sm">{timelineItem.description}</p>
                    </CardContent>
                  </Card>
                </div>
              ))}
      ...
      <main className="relative z-10 p-6">
        <article className="max-w-4xl mx-auto">
          ...
          <header className="mb-12">
            {/* Hero gradient */}
            <div className={`h-2 w-full bg-gradient-to-r ${post.heroGradient} mb-8 rounded-full`}></div>
